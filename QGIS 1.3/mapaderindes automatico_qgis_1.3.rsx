...
message("CREANDO CARPETA CON PAQUETES NECESARIOS")
ruta_libreria<-paste(path.expand('~'),"/mapaderindes",sep="")

if (dir.exists(ruta_libreria)) {
   print("La libreria de mapa de rindes existe")
} else {
  print("Generando libreria para mapa de rindes")
  dir.create(ruta_libreria,showWarnings = FALSE)
  print("Libreria creada en:")
  print(ruta_libreria)
  myPaths <- .libPaths()
  myPaths <- c(myPaths,ruta_libreria)
  .libPaths(myPaths)
}


#Se usa la libreria de mapa de rindes
.libPaths(ruta_libreria)
...
if("sp" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("sp",version= "1.4.5",lib=ruta_libreria)} else {print("sp ya esta..")}
if("gstat" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("gstat",version= "2.0.7",lib=ruta_libreria)} else {print("gstat ya esta..")}
if("spdep" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("spdep",version= "1.1.8",lib=ruta_libreria)} else {print("spdep ya esta..")}
if("raster" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("raster",version= "3.4.13",lib=ruta_libreria)} else {print("raster ya esta..")}
if("rgdal" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("rgdal",version="1.5.23",lib=ruta_libreria)} else {print("rgdal ya esta..")}
if("foreach" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("foreach", version= "1.5.1",lib=ruta_libreria)} else {print("foreach ya esta..")}
if("parallel" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("parallel",version="4.1.1",lib=ruta_libreria)} else {print("parallel ya esta..")}
if("doParallel" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("doParallel",version="1.0.16",lib=ruta_libreria)} else {print("doParallel ya esta..")}
if("doSNOW" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("doSNOW",version="1.0.19",lib=ruta_libreria)} else {print("doSNOW ya esta..")} 
if("RMySQL" %in% rownames(installed.packages(lib.loc=ruta_libreria)) == FALSE) {install.packages("RMySQL",version="0.10.22",lib=ruta_libreria)} else {print("RMySQL ya esta..")}
...
require(sp,lib.loc=ruta_libreria)
require(spdep,lib.loc=ruta_libreria)
require(gstat,lib.loc=ruta_libreria)
require(foreach,lib.loc=ruta_libreria)
require(parallel,lib.loc=ruta_libreria)
require(doParallel,lib.loc=ruta_libreria)
require(doSNOW,lib.loc=ruta_libreria)
require(raster,lib.loc=ruta_libreria)
require(RMySQL,lib.loc=ruta_libreria)
