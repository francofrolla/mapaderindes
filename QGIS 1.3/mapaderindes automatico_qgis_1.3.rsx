...
require(sf,lib.loc=ruta_libreria)
...
poligono <- raster::buffer(poligono, width=100) 
grdpts <- makegrid(poligono, cellsize = 5)
spgrd <- SpatialPoints(grdpts, proj4string = raster::crs((poligono)))
...
raster<- raster::raster(datoskriging,layer=1)
raster[raster==-9999]<-NA
raster::crs(raster)<-raster::crs(poligono)
