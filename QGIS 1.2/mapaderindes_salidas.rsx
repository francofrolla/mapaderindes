...
##Escala=selection quantiles(7);quantiles(4);15cm(15-120);500-4000(kg);1000-8000(kg);0.5-4(Tn);1-8(Tn); 
...
if(Escala == 3){version_simplificada = "na"}
if(Escala == 4){version_simplificada = "nu"}
if(Escala == 5){version_simplificada = "n1"}
if(Escala == 6){version_simplificada = "n2"}
...
print(paste("Metodo de salida grafica:",Escala))
print(paste("Metodo de salida grafica:",version_simplificada))
...
if (version_simplificada == "ni"){
    ...
    if (version_simplificada == "na"){
                    cols <- c("#AA0014","#d73027","#fc8d59","#fee08b","#d9ef8b","#91cf60","#1a9850","#0b5229")
             		values_tg<-c(500,1000,1500,2000,2500,3000,3500,4000)
                    cortes<-values_tg
                    lamatrix<-matrix(nrow=9,ncol=3)
    ...
    if (version_simplificada == "nu"){
                          cols <- c("#AA0014","#d73027","#fc8d59","#fee08b","#d9ef8b","#91cf60","#1a9850","#0b5229")
                   		values_tg<-c(1000,2000,3000,4000,5000,6000,7000,8000)
                          cortes<-values_tg
                          lamatrix<-matrix(nrow=9,ncol=3)
    ...
    if (version_simplificada == "n1"){
                                  cols <- c("#AA0014","#d73027","#fc8d59","#fee08b","#d9ef8b","#91cf60","#1a9850","#0b5229")
                           		values_tg<-c(0.5,1,1.5,2,2.5,3,3.5,4)
                                  cortes<-values_tg
                                  lamatrix<-matrix(nrow=9,ncol=3)
  ...
  if (version_simplificada == "n2"){
                                cols <- c("#AA0014","#d73027","#fc8d59","#fee08b","#d9ef8b","#91cf60","#1a9850","#0b5229")
                            values_tg<-c(1,2,3,4,5,6,7,8)
                                cortes<-values_tg
                                lamatrix<-matrix(nrow=9,ncol=3)
	  ...
	  proj4string(r) <- CRS(proj4string(poligono))
	  #crs(r) <- crs(Raster)
      ...
      proj4string(vectorizado) <- CRS(proj4string(poligono))
      ...
      print("Calculo de rendimiento medio por zona y desvio")
      medias<-c()
      desvio<-c()
      
	  newproj <- "+proj=longlat +datum=WGS84 +no_defs"
      capalote<- projectRaster(r,crs = newproj)
     
	  print(capalote)
	  print(vectorizado@data$output)


      for (i in (1:nrow(vectorizado@data))){
       zona1 <- vectorizado[vectorizado$output == vectorizado@data$output[i],]
       capa<-raster::crop(capalote,zona1)
       capa<-raster::mask(capa,zona1)

        mediazona<-cellStats(capa,"mean")
        sdzona<-cellStats(capa,"sd")
  
       medias<-c(medias,mediazona)
       desvio<-c(desvio,sdzona)
  
       }
	   
	   print(medias)
	   
      datos_pdf<-data.frame(round(medias,2),round(areas_lote/10000,2))
      colnames(datos_pdf)<-c("Rendimiento","Area (ha)")
	  
	  print("Calculo de toneladas producidas")
	  
	  granos_producidos<-round(sum(datos_pdf$Rendimiento*datos_pdf$`Area (ha)`),0)
	  
      
      print("Calculo de rendimiento medio ponderado y desvio")
      valores <- weighted.mean(datos_pdf$Rendimiento, datos_pdf$`Area (ha)`)
